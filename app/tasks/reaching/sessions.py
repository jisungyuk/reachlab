...
from tasks.reaching_task import TARGETS_SCREEN
...
COLUMNS    = ['#', 'Hand', 'Target ID', 'Hold (s)', 'Wait (s)', 'Move (s)', 'Instruction']
CSV_FIELDS = ['trial', 'hand', 'target_id', 'hold_s', 'wait_s', 'move_s', 'instruction']
NUM_COLS   = len(COLUMNS)
DEFAULTS   = ['1', '1', '1', '0.5', '2.0', '3.0', '1']
...
class SessionScreen(QWidget):
    ...
    def _on_back(self):
        target_screen = self.mw.screens.get(TARGETS_SCREEN)
        if target_screen and self.table.rowCount() > 0:
            valid_ids = set()
            for r in range(target_screen.table.rowCount()):
                item = target_screen.table.item(r, 0)
                if item:
                    valid_ids.add(item.text().strip())

            bad_trials = []
            for r in range(self.table.rowCount()):
                tid_item   = self.table.item(r, 2)
                trial_item = self.table.item(r, 0)
                if tid_item and tid_item.text().strip() not in valid_ids:
                    bad_trials.append(trial_item.text() if trial_item else str(r + 1))

            if bad_trials:
                QMessageBox.warning(
                    self, "Unregistered Targets",
                    f"The following trials reference a target ID not in the Targets list."
                    f"\n\nTrial: {', '.join(bad_trials)}"
                )
                return
    ...
    def _randomize(self):
        ...
        if n < 2:
            return
        rows = [[self.table.item(r, c).text() for c in range(NUM_COLS)] for r in range(n)]
        random.shuffle(rows)
        ...
        try:
            ...
            with open(path, 'r') as f:
                for row_data in csv.DictReader(f):
                    ...
                    self.table.setItem(r, 1, _cell(row_data.get('hand',        '1')))
                    self.table.setItem(r, 2, _cell(row_data.get('target_id',   '1')))
                    self.table.setItem(r, 3, _cell(row_data.get('hold_s',      '0.5')))
                    self.table.setItem(r, 4, _cell(row_data.get('wait_s',      '2.0')))
                    self.table.setItem(r, 5, _cell(row_data.get('move_s',      '3.0')))
                    self.table.setItem(r, 6, _cell(row_data.get('instruction', '1')))
